...
// === Markets ===============================================================
// Markets are a workspace-scoped variance axis — NOT a locale, brand, website,
// or scanner target. They let the same document resolve different content for,
// say, an EU vs. a CH audience. `country_code` is OPTIONAL advisory ISO-3166
// alpha-2 metadata consumed by Document Intelligence for country targeting; it
// is never a locale.
...
server.tool(
  ...
  "List markets in the active workspace — the workspace-scoped variance axis you can combine with brand/locale/site_profile when scoping overrides. Optionally filter by lifecycle status.",
  {
    status: z
      ...
      .optional()
      .describe("Filter by market lifecycle status."),
    ...paginationInput,
  },
...
server.tool(
  ...
  "Create a new market in the active workspace. `label` (the human name) is required; `code` is optional — the server suggests one from the label when omitted. Returns the persisted market.",
  {
    label: z
      ...
      .min(1)
      .max(200)
      .describe("Human display name of the market, e.g. 'European Union' (required)."),
    code: z
      .string()
      .max(48)
      .optional()
    ...
    description: z
      ...
      .max(2000)
      .optional()
      .describe("Optional free-text operator description."),
    country_code: z
      .string()
      .regex(/^[A-Za-z]{2}$/)
      .optional()
...
server.tool(
  ...
  "Edit a market's label, description, or country_code. The code is set at creation and is immutable thereafter (the server ignores it on update). Omitted fields are left untouched. Returns the updated market.",
  {
    ...
    label: z
      ...
      .max(200)
      .optional()
      .describe("Human display name of the market."),
    description: z
      .string()
      .max(2000)
      .optional()
      .describe("Free-text operator description."),
    country_code: z
      .string()
      .regex(/^[A-Za-z]{2}$/)
      .optional()
...
// === Site profiles =========================================================
// Site profiles are a second workspace-scoped variance axis (alongside markets)
// — NOT a locale, brand, or website. They let the same document resolve
// different content for, say, a 'b2c' vs. a 'b2b' presentation of the same
// brand. Combine with brand/market/locale when scoping overrides.
...
server.tool(
  ...
  "List site profiles in the active workspace — the second workspace-scoped variance axis. Optionally filter by lifecycle status.",
  {
    status: z
      .enum(["active", "inactive"])
      .optional()
      .describe("Filter by site-profile lifecycle status."),
    ...paginationInput,
  },
  async ({ status, page, per_page }) => {
    const params = new URLSearchParams();
...
server.tool(
  ...
  "Create a new site profile in the active workspace. `label` (the human name) is required; `code` is optional — the server suggests one from the label when omitted. Returns the persisted site profile.",
  {
    label: z
      ...
      .min(1)
      .max(200)
      .describe("Human display name of the site profile, e.g. 'B2C' (required)."),
    code: z
      .string()
      .max(48)
      .optional()
...
server.tool(
  ...
  "Edit a site profile's label or description. The code is set at creation and is immutable thereafter (the server ignores it on update). Omitted fields are left untouched. Returns the updated site profile.",
  {
    ...
    label: z
      ...
      .max(200)
      .optional()
      .describe("Human display name of the site profile."),
    description: z
      .string()
      .max(2000)
      .optional()
